...
namespace ToryFramework.Input
{
	...
	/// </summary>
	public class ToryVector4Input : ToryInput<Vector4>
	{
		#region CONSTRUCTOR

		protected ToryVector4Input() { }
		...
		OneEuroFilter[] oefs;
		Queue<Vector4> ensemble;
		Vector4 prevProcessedValue;
		...
		public override Vector4 RawValue 						{ get; protected set; }
		...
		public override Vector4 ProcessedValue 					{ get; protected set; }
		...
		public override ToryMultiInput<Vector4>[] MultiInputs 	{ get; protected set; }
		...
		protected override void Init()
		{
			...
			// Filters
			ensemble = new Queue<Vector4>();
			oefs = new OneEuroFilter[4];
			for (int i = 0; i < oefs.Length; i++)
			{
				oefs[i] = new OneEuroFilter(OEFFrequency.Value);
			}
			prevProcessedValue = ProcessedValue = RawValue = Vector4.zero;
		...
		public override void SetRawValue(Vector4 value, float timeStamp = -1f)
		{
			...
			switch (InteractionType.Value)
			{
				...
				case Input.InteractionType.DISCONTINUOUS:
					...
					resetValuesCrt = InputBehaviour.StartCoroutine(ManUtils.ManCoroutine.WaitAndAction(0.1f, () => RawValue = prevProcessedValue = ProcessedValue = Vector4.zero));
		...
		protected override Vector4 ApplyFilter(Vector4 value, float timeStamp = -1f)
		{
			Vector4 oefResult = Vector4.zero, eaResult = Vector4.zero;
			...
			Queue<Vector4>.Enumerator e = ensemble.GetEnumerator();
			while (e.MoveNext())
			{
				eaResult += e.Current;
			}
		...
		public override bool AddMultiInput()
		{
			...
			ToryMultiInput<Vector4> mi = new ToryVector4MultiInput();
		...
		public override bool AddMultiInput(int id)
		{
			...
			// Add a new multi-input.
			ToryMultiInput<Vector4> mi = new ToryVector4MultiInput(id);
			multiInputList.Add(mi);
			MultiInputs = multiInputList.ToArray();
			mi.ValueReceived += TriggerMultiInputValueReceivedEvent;
			mi.Interacted += TriggerMultiInputInteractedEvent;
		...
		public override bool RemoveMultiInput(ToryMultiInput<Vector4> input)
		{
			// Check the length.
		...
		public override bool RemoveMultiInput(int id)
		{
			...
			ToryMultiInput<Vector4> mi = multiInputList.Find( (obj) => { return obj.Id.Equals(id); } );
		...
		public override bool RemoveMultiInputAt(int i)
		{
			...
			ToryMultiInput<Vector4> mi = multiInputList.Find( (obj) => { return obj.Id.Equals(MultiInputs[i].Id); } );
			if (mi != null && multiInputList.Remove(mi))
			{
				MultiInputs = multiInputList.ToArray();
				mi.ValueReceived -= TriggerMultiInputValueReceivedEvent;
