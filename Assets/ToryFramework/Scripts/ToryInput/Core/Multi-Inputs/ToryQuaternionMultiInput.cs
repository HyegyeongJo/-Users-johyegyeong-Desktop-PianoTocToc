...
namespace ToryFramework.Input
{
	...
	/// </summary>
	public class ToryQuaternionMultiInput : ToryMultiInput<Quaternion>
	{
		#region CONSTRUCTOR

		public ToryQuaternionMultiInput() : base()
		{
			// Set a new id.
		...
		public ToryQuaternionMultiInput(int id) : base(id)
		{
			// Filters
			ensemble = new Queue<Quaternion>();
			oefs = new OneEuroFilter[4];
		...
		public override void SetRawValue(Quaternion value, float timeStamp = -1f)
		{
			...
			switch (InputBehaviour.InteractionType.Value)
			{
				case InteractionType.CONTINUOUS:
					...
					float d = Quaternion.Angle(prevProcessedValue, ProcessedValue) * InputBehaviour.Gain.Value / (curTime - prevTime);
