...
{
	/// <summary>
	/// The set of Vector3 values, which supports save and load functionalities.
	/// The value of this type can be serialized. (i.e., can be visualized in Unity editor.)
	/// </summary>
	[System.Serializable]
	public class ToryVector3 : ToryValue<Vector3>
	{
		#region FIELDS
		...
		[SerializeField] Vector3 savedValue;
		...
		public override Vector3 SavedValue
		{
			get
			{
				if (PlayerPrefsElite.VerifyVector3(KeyFormatter.GetSavedKey(Key)))
				{
					savedValue = PlayerPrefsElite.GetVector3(KeyFormatter.GetSavedKey(Key));
				}
				return savedValue;
			}
			set
			{
				...
				// Save the value.
				PlayerPrefsElite.SetVector3(KeyFormatter.GetSavedKey(Key), savedValue);
#if UNITY_EDITOR
				if (Application.isPlaying)
				{
#endif
		...
		public ToryVector3(string key) : this(key, Vector3.zero, Vector3.zero, Vector3.zero)
		{
		}

		public ToryVector3(string key, Vector3 value) : this(key, value, value, value)
		{
		}

		public ToryVector3(string key, Vector3 value, Vector3 defaultValue) : this(key, value, defaultValue, defaultValue)
		{
		}

		public ToryVector3(string key, Vector3 value, Vector3 defaultValue, Vector3 savedValue)
		{
			this.key = key;
		...
		public override bool LoadSavedValue()
		{
			if (PlayerPrefsElite.VerifyVector3(KeyFormatter.GetSavedKey(Key)))
			{
				Value = savedValue = PlayerPrefsElite.GetVector3(KeyFormatter.GetSavedKey(Key));
