...
namespace ToryValue.Editor 
{
	...
	[CustomPropertyDrawer(typeof(ToryFloat))]
	public class ToryFloatDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			// Get targets.
			ToryFloat[] targets = new ToryFloat[property.serializedObject.targetObjects.Length];
			for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
			{
				targets[i] = fieldInfo.GetValue(property.serializedObject.targetObjects[i]) as ToryFloat;
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("currentValue").floatValue });
					}
				}
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("defaultValue").floatValue });
					}
				}
			}
			...
			EditorGUI.BeginChangeCheck();
			{
				...
				if (!PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)) ||
				    !savedValueProperty.floatValue.Equals(
					    PlayerPrefsElite.GetFloat(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("savedValue").floatValue });
					}
				}
				...
				if (SecureKeysChecker.CheckSecureKeys())
				{
					...
					{
						PlayerPrefsElite.SetFloat(KeyFormatter.GetSavedKey(keyProperty.stringValue), 
						                          savedValueProperty.floatValue);
			...
			if (PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)))
			{
				if (!savedValueProperty.floatValue.Equals(
					PlayerPrefsElite.GetFloat(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
					savedValueProperty.floatValue = 
						PlayerPrefsElite.GetFloat(KeyFormatter.GetSavedKey(keyProperty.stringValue));
