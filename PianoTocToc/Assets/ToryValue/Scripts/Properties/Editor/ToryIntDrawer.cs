...
namespace ToryValue.Editor 
{
	...
	[CustomPropertyDrawer(typeof(ToryInt))]
	public class ToryIntDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			// Get targets.
			ToryInt[] targets = new ToryInt[property.serializedObject.targetObjects.Length];
			for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
			{
				targets[i] = fieldInfo.GetValue(property.serializedObject.targetObjects[i]) as ToryInt;
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("currentValue").intValue });
					}
				}
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("defaultValue").intValue });
					}
				}
			}
			...
			EditorGUI.BeginChangeCheck();
			{
				...
				if (!PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)) ||
					!savedValueProperty.intValue.Equals(
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("savedValue").intValue });
					}
				}
			...
			if (PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)))
			{
				if (!savedValueProperty.intValue.Equals(
					PlayerPrefsElite.GetInt(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
					savedValueProperty.intValue =
						PlayerPrefsElite.GetInt(KeyFormatter.GetSavedKey(keyProperty.stringValue));
