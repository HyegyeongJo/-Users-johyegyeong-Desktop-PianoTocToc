...
namespace ToryValue.Editor 
{
	...
	[CustomPropertyDrawer(typeof(ToryVector2))]
	public class ToryVector2Drawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			// Get targets.
			ToryVector2[] targets = new ToryVector2[property.serializedObject.targetObjects.Length];
			for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
			{
				targets[i] = fieldInfo.GetValue(property.serializedObject.targetObjects[i]) as ToryVector2;
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("currentValue").vector2Value });
					}
				}
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("defaultValue").vector2Value });
					}
				}
			}
			...
			EditorGUI.BeginChangeCheck();
			{
				...
				if (!PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)) ||
				    !savedValueProperty.vector2Value.Equals(
					    PlayerPrefsElite.GetVector2(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("savedValue").vector2Value });
					}
				}
				...
				if (SecureKeysChecker.CheckSecureKeys())
				{
					...
					{
						PlayerPrefsElite.SetVector2(KeyFormatter.GetSavedKey(keyProperty.stringValue), 
						                            savedValueProperty.vector2Value);
			...
			if (PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)))
			{
				if (!savedValueProperty.vector2Value.Equals(
					PlayerPrefsElite.GetVector2(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
					savedValueProperty.vector2Value = 
						PlayerPrefsElite.GetVector2(KeyFormatter.GetSavedKey(keyProperty.stringValue));
