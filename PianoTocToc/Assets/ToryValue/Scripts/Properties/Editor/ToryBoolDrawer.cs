...
namespace ToryValue.Editor 
{
	...
	[CustomPropertyDrawer(typeof(ToryBool))]
	public class ToryBoolDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			// Get targets.
			ToryBool[] targets = new ToryBool[property.serializedObject.targetObjects.Length];
			for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
			{
				targets[i] = fieldInfo.GetValue(property.serializedObject.targetObjects[i]) as ToryBool;
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("currentValue").boolValue });
					}
				}
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("defaultValue").boolValue });
					}
				}
			}
			...
			EditorGUI.BeginChangeCheck();
			{
				...
				if (!PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)) ||
				    !savedValueProperty.boolValue.Equals(
					    PlayerPrefsElite.GetBoolean(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { property.FindPropertyRelative("savedValue").boolValue });
					}
				}
				...
				if (SecureKeysChecker.CheckSecureKeys())
				{
					...
					{
						PlayerPrefsElite.SetBoolean(KeyFormatter.GetSavedKey(keyProperty.stringValue),
						                            savedValueProperty.boolValue);
			...
			if (PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)))
			{
				if (!savedValueProperty.boolValue.Equals(
					PlayerPrefsElite.GetBoolean(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
					savedValueProperty.boolValue = 
						PlayerPrefsElite.GetBoolean(KeyFormatter.GetSavedKey(keyProperty.stringValue));
