...
namespace ToryFramework.Editor
{
	...
	[CustomPropertyDrawer(typeof(ToryFilterTypeEnum))]
	public class ToryFilterTypeEnumDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			// Get targets.
			ToryFilterTypeEnum[] targets = new ToryFilterTypeEnum[property.serializedObject.targetObjects.Length];
			for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
			{
				targets[i] = fieldInfo.GetValue(property.serializedObject.targetObjects[i]) as ToryFilterTypeEnum;
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { (FilterType)property.FindPropertyRelative("currentValue").enumValueIndex });
					}
				}
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { (FilterType)property.FindPropertyRelative("defaultValue").enumValueIndex });
					}
				}
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { (FilterType)property.FindPropertyRelative("savedValue").enumValueIndex });
					}
				}
