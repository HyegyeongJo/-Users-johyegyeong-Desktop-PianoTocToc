...
namespace ToryFramework.Editor
{
	...
	[CustomPropertyDrawer(typeof(ToryInteractionTypeEnum))]
	public class ToryInteractionTypeEnumDrawer : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			// Get targets.
			ToryInteractionTypeEnum[] targets = new ToryInteractionTypeEnum[property.serializedObject.targetObjects.Length];
			for (int i = 0; i < property.serializedObject.targetObjects.Length; i++)
			{
				targets[i] = fieldInfo.GetValue(property.serializedObject.targetObjects[i]) as ToryInteractionTypeEnum;
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { (InteractionType)property.FindPropertyRelative("currentValue").enumValueIndex });
					}
				}
			}
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { (InteractionType)property.FindPropertyRelative("defaultValue").enumValueIndex });
					}
				}
			}
			...
			EditorGUI.BeginChangeCheck();
			{
				...
				if (!PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)) ||
				    !savedValueProperty.enumValueIndex.Equals(
					    PlayerPrefsElite.GetInt(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
			...
			if (EditorGUI.EndChangeCheck())
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (method != null)
					{
						method.Invoke(targets[i], new object[] { (InteractionType)property.FindPropertyRelative("savedValue").enumValueIndex });
					}
				}
				...
				if (SecureKeysChecker.CheckSecureKeys())
				{
					...
					{
						PlayerPrefsElite.SetInt(KeyFormatter.GetSavedKey(keyProperty.stringValue),
						                        savedValueProperty.enumValueIndex);
			...
			if (PlayerPrefs.HasKey(KeyFormatter.GetSavedKey(keyProperty.stringValue)))
			{
				if (!savedValueProperty.enumValueIndex.Equals(
					PlayerPrefsElite.GetInt(KeyFormatter.GetSavedKey(keyProperty.stringValue))))
				{
					savedValueProperty.enumValueIndex =
						PlayerPrefsElite.GetInt(KeyFormatter.GetSavedKey(keyProperty.stringValue));
