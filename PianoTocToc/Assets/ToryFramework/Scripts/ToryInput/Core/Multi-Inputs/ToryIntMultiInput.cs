...
namespace ToryFramework.Input
{
	...
	/// </summary>
	public class ToryIntMultiInput : ToryMultiInput<int>
	{
		#region CONSTRUCTOR

		public ToryIntMultiInput() : base()
		{
			// Set a new id.
		...
		public ToryIntMultiInput(int id) : base(id)
		{
			// Filters
			ensemble = new Queue<int>();
			oef = new OneEuroFilter(InputBehaviour.OEFFrequency.Value);
		...
		public override void SetRawValue(int value, float timeStamp = -1f)
		{
			...
			switch (InputBehaviour.InteractionType.Value)
			{
				case InteractionType.CONTINUOUS:
					...
					RawValue = value;
					ProcessedValue = (int)(ApplyFilter(value, timeStamp) * InputBehaviour.Gain.Value);
