...
namespace ToryFramework.Input
{
	...
	/// </summary>
	public class ToryFloatInput : ToryInput<float>
	{
		#region CONSTRUCTOR

		protected ToryFloatInput() { }
		...
		public override ToryMultiInput<float>[] MultiInputs 	{ get; protected set; }
		...
		public override bool AddMultiInput()
		{
			...
			ToryMultiInput<float> mi = new ToryFloatMultiInput();
		...
		public override bool AddMultiInput(int id)
		{
			...
			// Add a new multi-input.
			ToryMultiInput<float> mi = new ToryFloatMultiInput(id);
			multiInputList.Add(mi);
			MultiInputs = multiInputList.ToArray();
			mi.ValueReceived += TriggerMultiInputValueReceivedEvent;
			mi.Interacted += TriggerMultiInputInteractedEvent;
		...
		public override bool RemoveMultiInput(ToryMultiInput<float> input)
		{
			// Check the length.
		...
		public override bool RemoveMultiInput(int id)
		{
			...
			ToryMultiInput<float> mi = multiInputList.Find( (obj) => { return obj.Id.Equals(id); } );
		...
		public override bool RemoveMultiInputAt(int i)
		{
			...
			ToryMultiInput<float> mi = multiInputList.Find( (obj) => { return obj.Id.Equals(MultiInputs[i].Id); } );
			if (mi != null && multiInputList.Remove(mi))
			{
				MultiInputs = multiInputList.ToArray();
				mi.ValueReceived -= TriggerMultiInputValueReceivedEvent;
